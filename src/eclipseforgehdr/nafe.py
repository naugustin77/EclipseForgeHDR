...
def nafe_vn(A, sigma_sp=30.0, K=128, w=0.2, gamma=3.0, noise_sigma=None,
    ...
    WHICH WAY sigma GOES: the two papers contradict each other in one sentence
    and only one of them is right. 2013 p.3: "The higher the value of sigma is,
    the LOWER is the amount of noise in the low contrast parts of the image."
    2014 p.268: "The higher is the value of sigma, the HIGHER is the amount of
    noise". The 2013 wording is the correct one and the figures in BOTH papers
    agree with it -- their own captions run sigma = 10 for "an extreme
    enhancement" through sigma = 50 for "low-noise output images". It has to be
    that way round mechanically: a wider Gaussian on the cumulative histogram is
    more virtual noise added, which flattens the equaliser's response to the
    real noise and so shows less of it. Raising `noise_mult` here makes the
    layer QUIETER. Recorded because the 2014 chapter is the one with the
    variable neighbourhood, so it is the one a reader reaches for, and taken at
    its word it sends you the wrong way.
